Page({
  ...
  clickButton:function(event){
    ...
    if(id==this.data.id1){//退格
     ...
     data = data.substring(0,data.length-1);//截取到最后一位(不包含最后一项)
     if(data==""||data=="-"){
       data=0;
     }
     ...
     this.data.arr.pop();
        ...
        this.data.arr.length=0;
         ...
         this.setData({screenData:data});
          ...
          var lastIsOperator=data.substring(data.length-1,data.length);
          if (isNaN(lastIsOperator)){
            return;
          }
          var num="";
          var lastIsOperator;
          ...
          this.data.arr.length=0;
          wx.setStorageSync( "callogs",this.data.logs);//存储数据
